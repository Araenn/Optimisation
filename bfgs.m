...
function [x_estim, f] = BFGS(niter, critere, data, x0, Hu0)
    ...
    while erreur > critere
        ...
        alpha = linesearch(data, x0, d, 0.1);
...
function alpha = linesearch(data, x0, d, tol)
    f = @(alpha) fcout(data, x0 + alpha * d);  % Créez une fonction coût en fonction de alpha

    % Utilisez fminbnd pour trouver le minimum de la fonction de coût en fonction de alpha
